...
const lookupTimeseries = (country: string, metric: string, year: number): number => {
  ...
  for (let col = 3; col <= 53; col++) {
    ...
    const headerValue = getCell(Timeseries__Autogenerated_, cellKey)
  ...
  for (let row = 2; row <= 10000; row++) {
    const countryCell = getCell(Timeseries__Autogenerated_, `A${row}`)
    const metricCell = getCell(Timeseries__Autogenerated_, `B${row}`)
    ...
    if (cleanCountry === normalizedCountry && cleanMetric === searchMetric) {
      ...
      const value = getCell(Timeseries__Autogenerated_, cellKey)
